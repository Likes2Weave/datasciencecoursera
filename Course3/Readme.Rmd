...
Based on features_info.txt, observed features representing mean or standard deviation include the string "mean()" or "std()" in their name. Features that included "Mean" as part of an ordered pair were excluded since they are inputs to the angle measurements and not discrete entities.

Tidyframe includes variable names, use grep to extract a list of columns containing the target strings. 
...
There were 79 total occurrences of the strings "mean()" and "std()" in the original features.txt file.  Including the three fixed variables (original_file, activity_number, subject) the final data frame correctly includes 82 variables.

# 3. Use descriptive activity names to name the activities in the data set

An activity id/number and name is listed in activity_labels.txt for each unique activity in the data set. Read the activity_labels.txt file into a table to use with a join to replace activity numbers with names in tidyframe.  
...
Assured there are no missing values use mutate to add activity name, which is more descriptive that a number.  
...
``` {r build activity_labels table}
    ...
    activity_tidyframe <- select (activity_tidyframe, original_file, activity_name, subject, 4:82)
    
    str(activity_tidyframe, list.len = 5)
```

The new data frame activity_tidyframe includes a fixed variable, activity_name, to describe the activities in the data set.
...
Earlier, it was discovered there are three groups of 14 identical variables that start with "fBodyAcc-bandsEnergy()" and end with an ordered pair in lines 303 through 344 of features.txt. The only way to make these names unique was to include the row number. Are any of these variables included in activity_tidyframe?

```{confirm possible duplicates not included}
    grep("fBodyAcc-bandsEnergy()", names(activity_tidyframe), value = TRUE)
```

None of these variables are included with the mean() or std() calculated measurements extracted to create activity_tidyframe. They do not need to be included in the standardizing process for the activity_tidyframe data set. The leading digits can be removed.

### Standardized capitals in variable names

According to Wickham there are several approaches to creating variable names.  The UCI HAR Dtaset authors chose the camelback style... for data frame variables and ... for file names. With the exception of data frame and vectors used internally in run_analysis.R, their standard was followed for this assignment.

Because special characters like dashes and parentheses can create problems, use gsub to remove or replace them.

``` {r gsub comma, parentheses}
...
``` {r test regex to remove digits}
    find_digits <- grep("^[0-9]{1,3} ", names_no_paren, value = TRUE)
    str(find_digits, list.len = 5 )
```

Looks like we have 79 variables with leading digits plus a space. Looks fine: 79 variables, three fixed, yields 82 total. Use gsub to remove leading digits and space and update variable names.
...
## First Iteration - Measured features represent a single value
...
With thirty subjects and six activities, final_tidyframe_1 should have 180 rows.  The original_file info was not needed, and was omitted from summary data, resulting in 81 columns.

This approach is appropriate if the data set consumer considered each of the 79 measured variables a single unit of data.  Grouping by subject and then activity created a data set that could be used as is, or to further summarize data by additional combinations of subject or activity.

## Second Iteration - Measured features represent more than one value

There may be some consumers that would prefer a wider data set.  Looking at the data frame extracted in Part 4, the mean() and std() variables contain three segments: feature, calculation and reference. For tBodyAcc_mean_X, tBodyAcc would be the feature, mean would be the calculation and X would be the reference.  Summarized: 
...
The data frame could be reshaped to allow feature variable grouping by subject and activity, as well as calculation and/or reference. Grouping by all four would yield summary data equivalent to the first iteration above. However, not all possible combination of feature, calculation and reference are referenced by the data.  Introduce "NA" to reflect missing elements and must strexclude NA's from mean calculations.

## Summary: A tidy data set

To meet the rubric, the data set may be narrow or wide, as long as it is tidy. A tidy data set must meet all three of Wickham's conditions:  

    1. Each variable forms a column.  
        - A variable is a measured feature as recorded
        - A Variable is a specific feature segmented by calculation and reference
  
    2. Each observation forms a row. 
        - An observation is one subject, activity, and set of recorded features
        - An observation is one subject, activity, calculation, reference and feature

    3. Each type of observational unit forms a table.  
        - In both cases, the observational units are consistent

Whether the first or second data set would be considered tidy, would be based the consumer's definition of variable: as recorded or further segmented by calculation and reference. 

Wickham and Hood discussed that "tidy-ness" is a function of how well the data set fits the task.  For looking at the variable averages grouped by subject and activity, both iterations would work.  If the consumer needed more grouping options, the second would provide the greatest flexibility.  Thus the second one will be uploaded to complete the assignment.  

### Cautions

As with creating descriptive varaibles, there is a trade off between using data as originally recorded and data as segmented. R offers power tools for reshaping data, but just because one can reshape the data, does not mean the reshaped version is meaningful.  Reshaping data sets must be accompanied by a solid knowledge of the derivation of the data and its applications.  

Consider an example, with a quick look through my pantry an observer might record   
...
It might be tempting to re-organize my pantry storage by storage type: bags, bins, baskets, cans, jars.  It would be very tidy, but a total disaster for meal prep because it is not the container that is critical, but the contents and applications.  This would be a more valid recording of my pantry:
...
I definitely do not want the dog food next to the refried beans, or the pumpkin seeds with the mung beans!

Returning to the submitted data set, although the second iteration appears to be the tidy-est, segmenting the feature variables may or may not add to the value of the data set. To make sure the consumer could recontruct the origonal data set, I added a key with the original variable name in the codebook.  
...
Lionel Henry & Hadley Wickham, "Tidy Evaluation" at https://tidyeval.tidyverse.org/index.html (04/09/2019)
